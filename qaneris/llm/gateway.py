...
``aiyallm`` performs model invocation and nothing else. Everything that defines Qaneris's
...
from qaneris.contracts import BusinessQuery, QueryPlanStep
...
def build_chat_client(
    *,
    ...
    """Build the aiyallm client for the one OpenAI-compatible endpoint Qaneris is configured with.
...
class AiyallmSchemaModel:
    """The Qaneris model port, carried by aiyallm.

    The configured endpoint is described by Qaneris's own ``QANERIS_MODEL_*`` variables; how
    ...
    def from_environment(cls) -> AiyallmSchemaModel | None:
        ...
        Which set is selected is decided in :mod:`qaneris.llm.config`; an unconfigured model
    ...
    def _chat(self, system: str, user: str, json_response: bool = False) -> str:
        ...
        Qaneris's own prompt asks for a bare JSON object, and the parse + validate step in the
    ...
    def propose_evidence_questions(self, context):
        from qaneris.diagnostics.models import DiagnosticDecision
