...
Every tool here is a thin projection of one ``QanerisService`` public method. The MCP boundary
...
* **Error translation** - a ``QanerisError`` keeps its stable code and redacted message; any
...
mcp = FastMCP("Qaneris")
service = QanerisService.from_environment()
...
def _publishable_error(error: Exception) -> Exception:
    ...
    A ``QanerisError`` already carries a stable machine code and a message the application
    ...
    if isinstance(inner, ToolFailure):
        return inner
    if isinstance(inner, QanerisError):
...
async def list_adapters() -> str:
    """List database kinds and drivers supported by this Qaneris installation."""
...
def _drive_ask_stream(
    ...
    ``QanerisService.ask_stream()`` is a plain ``Iterator``, so it must not be driven on the event
...
async def ask_data(
    ...
    MCP tool error, carrying the stable Qaneris code the service already published.
    """
    request = AskRequest(
        question=question,
        workspace_id=workspace_id,
