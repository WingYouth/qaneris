...
The endpoint calls ``QanerisService.import_excel()`` and nothing else. It never touches the
...
from qaneris.common.errors import DatasourceUnavailableError
from qaneris.common.redaction import SecretRedactor
from qaneris.contracts import ErrorDetail, ErrorResponse
...
def excel_error_response(error: ExcelIngestionError) -> JSONResponse:
    ...
    cell value or a resolved credential. A generic ``QanerisError`` handler would flatten every
