...
planner, no compiler and no executor: it asks ``QanerisService.ask_stream()`` for events and puts
...
from qaneris.contracts import AskEvent, AskRequest
...
async def stream_ask_events(
    service: QanerisService, request: AskRequest, http_request: Request
) -> AsyncIterator[str]:
