...
"""The formal ``qaneris ask`` product command.

One command maps to one :class:`AskRequest` and one :class:`AskResponse`. The command is a thin
boundary over ``QanerisService.ask()``: it owns argument parsing, exit codes and rendering, and
...
``QanerisService.ask_stream()``. The CLI owns no event schema of its own - it serializes the
...
from qaneris.cli.render import render_table
...
from qaneris.contracts.api import AskRequest, AskResponse, AskStatus
