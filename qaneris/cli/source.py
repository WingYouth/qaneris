...
from qaneris.cli.config import (
    DatasourceConfigError,
    load_datasource_requests,
    load_single_datasource_request,
)
from qaneris.cli.render import operation_error_detail, render_table
...
from qaneris.contracts.connection import SecureDatasourceTest, SecureDatasourceUpdate
from qaneris.ingestion.excel import ExcelImportRequest, ExcelImportResult
...
def application_service() -> QanerisService:
    """Build the Application Service from the runtime environment.

    The catalog follows the same ``QANERIS_CATALOG`` convention that ``doctor``, the API and the
    ...
    """
    return QanerisService(Catalog(os.getenv("QANERIS_CATALOG", "qaneris.db")))
...
def _scan_one(service: QanerisService, datasource_id: str) -> dict[str, Any]:
...
def run_catalog_command(args: argparse.Namespace) -> int:
    ...
    Every branch is a thin projection over ``QanerisService``: the CLI writes no catalog query, so
...
def print_datasource_created(datasource: Datasource, *, json_output: bool) -> None:
    ...
    print(f"Next: qaneris source scan-one {datasource.id}")
