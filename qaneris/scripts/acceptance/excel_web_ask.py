...
``QANERIS_MODEL_BASE_URL`` / ``QANERIS_MODEL_API_KEY`` / ``QANERIS_MODEL_NAME``)::
    ...
    set -a && . ./.env && set +a
    python3 -m qaneris.scripts.acceptance.excel_web_ask

Unlike the EXCEL-01A acceptance, which drives ``QanerisService`` in-process, and the EXCEL-01B one,
...
from qaneris.graph import Neo4jConfig, graph_reader_from_environment
from qaneris.ingestion.excel import DEFAULT_EXCEL_POLICY, artifact_key
