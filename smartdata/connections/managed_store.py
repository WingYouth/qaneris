...
_AAD_TEMPLATE = "smartdata-managed-secret-v1|{secret_id}|{kind}"
...
class ManagedCredentialStore:
    ...
    def from_environment(
        ...
        *,
        ...
        """Build the store from ``SMARTDATA_SECRET_STORE_DIR`` and ``SMARTDATA_MASTER_KEY``.
    ...
    def _store_directory(env: Mapping[str, str]) -> Path:
        ...
        if store_dir == repository or repository in store_dir.parents:
            raise CredentialStoreConfigurationError(
                f"{STORE_DIR_ENV} must be outside the SmartData project directory"
            )
        return store_dir
