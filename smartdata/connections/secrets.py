...
class ManagedSecretProvider:
    ...
    environment and file references must keep starting without ``SMARTDATA_MASTER_KEY`` and
    ``SMARTDATA_SECRET_STORE_DIR`` configured, so the configuration is required exactly when a
