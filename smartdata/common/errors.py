...
class SmartDataError(ValueError):
    code = "smartdata_error"
    status_code = 400
...
class DatasourceNotFoundError(SmartDataError):
    code = "datasource_not_found"
    status_code = 404


class DatasourceUnavailableError(SmartDataError):
    code = "datasource_unavailable"


class DatasourceConnectionTestError(SmartDataError):
    ...
    one SmartData error there and sanitized on the way in, so a driver's raw exception type and
...
class DatasourceSecureProfileRequiredError(SmartDataError):
...
class QueryPlanningError(SmartDataError):
...
class IntentParsingError(SmartDataError):
    code = "intent_parsing_failed"


class ModelInvocationError(SmartDataError):
    ...
    SmartData error there, so no provider SDK exception type reaches the semantic, querying or
...
class QuerySafetyError(SmartDataError):
    code = "unsafe_query"


class ExecutionValidationError(SmartDataError):
    code = "execution_validation_failed"


class QueryExecutionError(SmartDataError):
    code = "execution_failed"


class CredentialStoreConfigurationError(SmartDataError):
    """The managed credential store was asked to work without a usable configuration.

    Raised for a missing ``SMARTDATA_SECRET_STORE_DIR`` / ``SMARTDATA_MASTER_KEY``, for a store
...
class ManagedSecretNotFoundError(SmartDataError):
...
class ManagedSecretInUseError(SmartDataError):
...
class ManagedSecretIntegrityError(SmartDataError):
...
class CredentialUploadTooLargeError(SmartDataError):
...
class CredentialKindMismatchError(SmartDataError):
...
class CertificateValidationError(SmartDataError):
...
class TLSMaterializationError(SmartDataError):
...
class TLSFeatureUnsupportedError(SmartDataError):
