...
Run it with a real Neo4j reachable (``SMARTDATA_GRAPH_STORE=neo4j`` plus the ``SMARTDATA_NEO4J_*``
credentials) from the project root::

    python3 -m smartdata.scripts.acceptance.import_excel
...
def attempt(service: SmartDataService, request: ExcelImportRequest) -> ExcelIngestionError | None:
...
def main() -> int:
    ...
    service = SmartDataService(Catalog(run_root / "catalog.db"))
    ...
    alternate = ExcelIngestionService(
        SmartDataService(Catalog(run_root / "catalog-alternate.db")), policy=alternate_policy
    )
    ...
    no_graph_service = SmartDataService(
    ...
    scan_failure_service = SmartDataService(
    ...
    dropped_service = SmartDataService(
