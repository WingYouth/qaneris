...
3. **One service call.** The route calls ``SmartDataService`` and nothing else. It never touches the
...
def service_of(request: Request) -> SmartDataService:
    ...
    The credential boundary never builds a second ``SmartDataService``, catalog or credential
