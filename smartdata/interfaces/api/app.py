...
from smartdata.llm.gateway import summarize_without_model
from smartdata.querying.retrieval.schema_context import build_schema_context


def create_app(database_path: str | None = None) -> FastAPI:
    path = database_path or os.getenv("SMARTDATA_CATALOG", "smartdata.db")
    service = SmartDataService(Catalog(path))
    app = FastAPI(title="SmartData API", version="0.1.0")
    ...
    @app.exception_handler(SmartDataError)
    async def smartdata_error_handler(_: Request, error: SmartDataError) -> JSONResponse:
    ...
    async def stream_ask(request: AskRequest, http_request: Request) -> StreamingResponse:
        ...
        events carried here are the same ``AskEvent`` objects ``SmartDataService.ask_stream()``
...
def main() -> None:
    uvicorn.run("smartdata.interfaces.api.app:app", host="127.0.0.1", port=8000, reload=False)
