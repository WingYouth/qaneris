...
SMARTDATA_MODEL_PROFILE=modelscope

# A named set: SMARTDATA_MODEL_<NAME>_{BASE_URL,API_KEY,NAME}
SMARTDATA_MODEL_MODELSCOPE_BASE_URL=https://api-inference.modelscope.cn/v1
SMARTDATA_MODEL_MODELSCOPE_API_KEY=...
...
PREFIX = "SMARTDATA_MODEL_"

#: Names the set to use. Not a profile field of its own.
PROFILE_SELECTOR = "SMARTDATA_MODEL_PROFILE"
...
#: ``SMARTDATA_MODEL_PROFILE`` occupies the ``PROFILE`` slot, so no set may claim that name.
...
def profile_names(env: Mapping[str, str] | None = None) -> list[str]:
    ...
    default set (``SMARTDATA_MODEL_BASE_URL``) leaves an empty name and is not listed.
    """
