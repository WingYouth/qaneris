...
class Neo4jConfig:
    ...
    def from_environment(cls, environment: Mapping[str, str] | None = None) -> Neo4jConfig:
        ...
        names = {
            "uri": "SMARTDATA_NEO4J_URI",
            "username": "SMARTDATA_NEO4J_USERNAME",
            "password": "SMARTDATA_NEO4J_PASSWORD",
        }
        ...
        return cls(
            **values,
            database=env.get("SMARTDATA_NEO4J_DATABASE", "neo4j").strip() or "neo4j",
        )
...
def graph_store_from_environment(
    ...
    env = environment if environment is not None else os.environ
    backend = env.get("SMARTDATA_GRAPH_STORE", "neo4j").strip().lower()
    if backend == "null":
        return NullGraphStore()
    if backend != "neo4j":
        raise ValueError(f"Unsupported SMARTDATA_GRAPH_STORE: {backend}")
...
def graph_reader_from_environment(
    ...
    """Read side factory sharing the same ``SMARTDATA_GRAPH_STORE`` switch as the write side."""
    env = environment if environment is not None else os.environ
    backend = env.get("SMARTDATA_GRAPH_STORE", "neo4j").strip().lower()
    if backend == "null":
