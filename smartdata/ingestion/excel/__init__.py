"""EXCEL-01A core Excel ingestion: ``.xlsx`` → immutable SQLite → existing SmartData chain.
...
Interface layers go through ``SmartDataService.import_excel`` and never touch the validator, the
materializer or the artifact layout directly.
"""

from smartdata.ingestion.excel.contracts import (
...
from smartdata.ingestion.excel.materializer import (
    SQLiteMaterializer,
    TableWrite,
    quote_identifier,
)
from smartdata.ingestion.excel.policy import (
    DEFAULT_EXCEL_POLICY,
    ExcelIngestionPolicy,
    artifact_key,
    policy_digest,
)
from smartdata.ingestion.excel.service import ExcelIngestionService
from smartdata.ingestion.excel.types import (
    CellKind,
    ColumnType,
...
from smartdata.ingestion.excel.validator import (
    ColumnPlan,
    ContainerInfo,
    SheetPlan,
    WorkbookPlan,
