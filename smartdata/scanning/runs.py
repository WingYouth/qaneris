...
from smartdata.common.redaction import SecretRedactor, safe_error
...
from smartdata.contracts.datasource import DatasetInfo, RelationInfo
from smartdata.contracts.profile import ScanSnapshot
from smartdata.graph.reading import GraphStructureRequest
from smartdata.profiling.documents import FileSystemProfileDocumentWriter
...
class SnapshotGraphValidator:
    ...
    @staticmethod
    def _workspace_id(service: SmartDataService, datasource_id: str) -> str:
...
class MultiDatabaseScanRunner:
    ...
    def __init__(
        ...
        *,
        ...
        service_factory: Callable[[Catalog], SmartDataService] | None = None,
        ...
        self.service_factory = service_factory or (
            lambda catalog: SmartDataService(catalog, secret_resolver=self.secret_resolver)
        )
    ...
    def scan(
        ...
        *,
        ...
        service: SmartDataService | None = None
        try:
            try:
    ...
    def _scan_one(
        self, service: SmartDataService, request: SecureDatasourceCreate
    ) -> ScanRunMember:
    ...
    def _markdown(report: dict[str, Any]) -> str:
        ...
        lines = [
            "# SmartData Multi-Database Scan Report",
            "",
            "## Run",
            "",
            f"- Run ID: `{run['run_id']}`",
    ...
    @staticmethod
    def _close_service(service: SmartDataService | None) -> None:
        if service is None:
            return
        seen: set[int] = set()
