# SmartData Next-Generation Architecture & Implementation Plan(智能数据平台下一代架构与实施计划)
...
> 当前四天优先级是 Roadshow Release：16 个服务型测试库服务器部署与 SmartData 验收、14 个 SSL/TLS、证书生命周期、Excel、Web / CLI / Skill / MCP、Streaming、Trace / SQL、Visualization 与演示视频。
> 当前实现与下一步以 [Development Roadmap](development-roadmap.md) 和 `main` 源码为准。

# 0. 文档定位

本文件是 SmartData(智能数据平台) 当前开发的权威主架构文档。后续模块设计不得与本文件冲突。
...
- 16 个服务型测试数据库从 `NLQuery-Test-Dataset` 部署到路演服务器，并由 SmartData 主仓库完成真实 connection + scan + Neo4j 回读验收。
...
- Phase 1 Trusted Query Engine：`SmartDataService.ask()` 主链已完成。
- Scan → Neo4j Enterprise Data Graph、多数据源 ScanRun、ScanSnapshot 与图回读校验已实现。
- API `POST /api/ask` 与 MCP `ask_data` 已存在。
- CLI 已实现 `doctor`、Phase 1 acceptance、`source test/scan`、`source import-excel` 与非交互式 `smartdata ask`（human/JSON）。
...
```text
16 service DB server deployment + real SmartData scan acceptance
+ 14 SSL/TLS
...
- CLI `smartdata ask` 已实现非交互式 human/JSON 输出（RS-CLI-01A）；交互式 REPL 不在路演范围。统一 Streaming Event Contract 尚未建立。
...
SmartData 已完成第一阶段“可信地问数据库”的核心闭环：
