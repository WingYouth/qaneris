...
脚本读取仓库根目录的 `.env`（如果存在），在 `127.0.0.1:8000` 启动 FastAPI、在 `127.0.0.1:5173` 启动 Vite，并打开浏览器。也可以运行 `./start-web.sh --no-open`。按 Ctrl+C 停止脚本启动的进程；如果端口已有服务，脚本会复用。脚本会检查后端是否提供对话删除接口；若当前仓库的旧版后端缺少该接口，会重启它。无法确认进程归属时会报错，不会误报启动成功。上述安装命令已包含页面支持的数据库驱动；在本机连接 SQL Server 还需要系统安装 ODBC Driver 18 for SQL Server。
