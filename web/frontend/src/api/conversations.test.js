...
import { cancelRun, clarifyRun, createConversation, createRun, deleteConversation, getConversation, getRun, listConversations, retryRun } from "./conversations.js";
...
test("conversation and run clients use only relative URLs and the contracted bodies", async () => {
  ...
  assert.deepEqual(calls.map((call) => [call.method, call.url]), [
    ...
    ["DELETE", "/api/conversations/conv%2F1"],
  ]);
...
test("delete explains a stale backend that lacks the DELETE route", async () => {
  globalThis.fetch = async () => new Response('{"detail":"Method Not Allowed"}', {
    status: 405, headers: { "content-type": "application/json" },
  });
  await assert.rejects(deleteConversation("old"), (error) =>
    error.code === "conversation_delete_unavailable" && error.message.includes("重启后端"));
});
