...
export function DatasourceWizard({ workspaceId = "default", onWorkspaceChange, existing, onCancel, onSaved }) {
  ...
  return <div className="connection-flow">
    ...
    <header className="connection-flow__heading"><div className="connection-flow__heading-main"><span className="connection-flow__driver-mark" aria-hidden="true">{driverName(form.driver).replace(/[^A-Za-z]/g, "").slice(0, 2).toUpperCase()}</span><div><h1>{existing ? `重新连接 ${driverName(form.driver)}` : `${driverName(form.driver)} 连接`}</h1><p>{isSaved ? "连接设置已保存。完成扫描后即可在工作区使用。" : "填写连接信息与证书，再从 SmartData 后端测试连接。"}</p></div></div><span>{isSaved ? "03 / 03 · 扫描" : "02 / 03 · 连接设置"}</span></header>
          ...
          {tab === "basic" ? <div className="connection-editor__layout"><div className="connection-editor__primary">
            ...
            <section className="connection-editor__section"><h2>服务器</h2>{!SPECIAL_ENDPOINT.has(form.driver) && !HOST_ONLY.has(form.driver) ? <div className="connection-editor__segment" role="group" aria-label="连接方式"><button type="button" className={form.locatorMode === "host" ? "is-active" : ""} aria-pressed={form.locatorMode === "host"} onClick={() => update("locatorMode", "host")}>Host + Port</button><button type="button" className={form.locatorMode === "url" ? "is-active" : ""} aria-pressed={form.locatorMode === "url"} onClick={() => update("locatorMode", "url")}>URL</button></div> : null}
              <div className="connection-editor__fields">{form.driver === "sqlite" ? textInput("path", "服务器可访问的 SQLite 路径", { placeholder: "/data/example.db" }) : form.driver === "bigquery" ? <>{textInput("project", "Project")}{textInput("namespace", "Dataset")}</> : form.driver === "snowflake" ? <>{textInput("account", "Account")}{textInput("database", "Database")}{textInput("namespace", "Schema")}</> : <>
                ...
                {form.driver === "cassandra" ? textInput("namespace", "Keyspace", { hint: "连接由 SmartData 后端发起。" }) : form.driver !== "hbase" ? textInput("database", "数据库", { hint: "连接由 SmartData 后端发起；TLS 主机名须与证书匹配。" }) : null}
              </>}</div></section>
            ...
            </div></section></div><aside className="connection-editor__aside"><span className="eyebrow">连接路径</span><h3>从后端验证连接</h3><p>SmartData 后端直接连接数据库。浏览器不会接收数据库密码。</p><div className="connection-editor__flow"><span>浏览器</span><i>→</i><span>后端</span><i>→</i><span>{driverName(form.driver)}</span></div><p>如服务器要求证书，请切换到“SSL / 证书”上传 CA、客户端证书和私钥。</p></aside></div> : null}
    ...
    {!isSaved && error ? <div className="connection-feedback connection-feedback--error" role="alert"><strong>连接未通过</strong><p>{error.message}</p>{error.code === "credential_store_configuration_error" ? <small>请先在后端配置 SMARTDATA_SECRET_STORE_DIR 和 SMARTDATA_MASTER_KEY，然后重启后端。</small> : null}</div> : null}
