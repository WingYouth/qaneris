...
export function DatasourceManager({ workspaceId = "default", onAsk, onWorkspaceChange }) {
  ...
  return <div className="source-layout">
    <section className="surface-card source-panel" aria-labelledby="source-heading">
      ...
      {items.length ? <div className="source-list">{items.map((item) => <article className="source-item" key={item.id}>
      ...
      </article>)}</div> : <div className="source-empty"><strong>还没有数据源</strong><p>添加数据库连接，或从 Excel 导入工作簿。</p></div>}
    </section>
