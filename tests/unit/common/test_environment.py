...
def test_explicit_env_file_wins_over_selector_and_cwd(tmp_path, monkeypatch) -> None:
    ...
    cwd = tmp_path / ".env"
    explicit.write_text("SMARTDATA_ENV_TEST=explicit\n", encoding="utf-8")
    selected.write_text("SMARTDATA_ENV_TEST=selected\n", encoding="utf-8")
    cwd.write_text("SMARTDATA_ENV_TEST=cwd\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("SMARTDATA_ENV_FILE", str(selected))
    monkeypatch.delenv("SMARTDATA_ENV_TEST", raising=False)

    loaded = load_runtime_environment(explicit)

    assert loaded == explicit.resolve()
    assert __import__("os").environ["SMARTDATA_ENV_TEST"] == "explicit"


def test_env_file_selector_wins_over_cwd(tmp_path, monkeypatch) -> None:
    selected = tmp_path / "selected.env"
    selected.write_text("SMARTDATA_ENV_TEST=selected\n", encoding="utf-8")
    (tmp_path / ".env").write_text("SMARTDATA_ENV_TEST=cwd\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
    monkeypatch.setenv("SMARTDATA_ENV_FILE", str(selected))
    monkeypatch.delenv("SMARTDATA_ENV_TEST", raising=False)

    assert load_runtime_environment() == selected.resolve()
    assert __import__("os").environ["SMARTDATA_ENV_TEST"] == "selected"


def test_cwd_dotenv_does_not_override_process_environment(tmp_path, monkeypatch) -> None:
    (tmp_path / ".env").write_text("SMARTDATA_ENV_TEST=dotenv\n", encoding="utf-8")
    monkeypatch.chdir(tmp_path)
    monkeypatch.delenv("SMARTDATA_ENV_FILE", raising=False)
    monkeypatch.setenv("SMARTDATA_ENV_TEST", "process")

    assert load_runtime_environment() == tmp_path / ".env"
    assert __import__("os").environ["SMARTDATA_ENV_TEST"] == "process"


def test_selected_missing_file_is_configuration_error(tmp_path, monkeypatch) -> None:
    monkeypatch.setenv("SMARTDATA_ENV_FILE", str(tmp_path / "missing.env"))
...
def test_missing_cwd_dotenv_is_normal_and_parent_is_not_searched(tmp_path, monkeypatch) -> None:
    ...
    (parent / ".env").write_text("SMARTDATA_ENV_TEST=parent\n", encoding="utf-8")
    monkeypatch.chdir(child)
    monkeypatch.delenv("SMARTDATA_ENV_FILE", raising=False)
    monkeypatch.delenv("SMARTDATA_ENV_TEST", raising=False)

    assert load_runtime_environment() is None
    assert "SMARTDATA_ENV_TEST" not in __import__("os").environ
